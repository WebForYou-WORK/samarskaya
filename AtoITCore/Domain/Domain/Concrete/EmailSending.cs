...
namespace Domain.Concrete
{
    ...
    public class EmailSending : IEmailSending
    {
        ...
        /// </summary>
        public  void SendMailToAdministrator(Basket basket, OrderDetails details, string mailFrom, string mailPasword, string hostName, int port, bool enableSsl, string mailAdmin, string attachFile = null)
        {
            try
            {
                //c какой почты отправляем письмо 
                MailMessage mail = new MailMessage { From = new MailAddress(mailFrom) };
                //("samarskaya918@gmail.com")); // E-mail Администратора
        ...
        /// </summary>
        public void SendMail(Basket basket, OrderDetails details, string mailFrom, string mailPasword, string hostName, int port, bool enableSsl, string attachFile = null)
        {
            try
            {
                //c какой почты отправляем письмо
                ...
                mail.Subject = "samarskaya-designer.com"; // тема письма
            ...
            catch (Exception)
            {
                ...
                basket.AnswerList.Add("");
                basket.AnswerList.Add("Заказ отправлен администратору в обработку, скоро мы с Вами свяжемся, хорошего Вам дня!");
            }
        }
        ...
        public static string EmailMessage(Basket basket, OrderDetails details)
        {
            ...
            str.AppendLine("");
            str.AppendLine("Благодарим Вас за покупку на сайте - samarskaya-designer.com !");
            str.AppendLine("");
            str.AppendLine("");
        ...
        public static string EmailMessageToAdministrator(Basket basket, OrderDetails details)
        {
            ...
            str.AppendLine("Вам с сайта samarskaya-designer.com поступил новый заказ!");
            str.AppendLine("");
            str.AppendLine("");
